...
export function ridgeChart(vol, { width = 352, height = 158, selected = null, colors = {}, labelEvery = 0 } = {}) {
  ...
  const RUN = colors.run || CY, RIDE = colors.bike || BIKE, HIKE = colors.hike || "#7fd6c0", GYM = colors.gym || "#c98bdb", SWIM = colors.swim || "#46c7e6";
  const T = p => p.run + p.bike + (p.hike || 0) + (p.gym || 0) + (p.swim || 0);
  ...
  vol.forEach((p, i) => {
    const x = i * (bw + gap), runTop = p.bike + p.run, hikeTop = runTop + (p.hike || 0), gymTop = hikeTop + (p.gym || 0), tot = T(p);
    const dim = selected != null && selected !== i;
    const op = (p.isDeload ? 0.45 : 1) * (dim ? 0.45 : 1);
    const lay = (yTop, yBot, col, top) => `<rect x="${x}" y="${yTop}" width="${bw}" height="${Math.max(0, yBot - yTop)}" ${top ? "rx=\"2.5\"" : ""} fill="${col}"/>`;
    s += `<g opacity="${op}">`;
    s += lay(y(p.bike), base, RIDE, false);
    s += lay(y(runTop), y(p.bike), RUN, !p.hike && !p.gym && !p.swim);
    if (p.hike) s += lay(y(hikeTop), y(runTop), HIKE, !p.gym && !p.swim);
    if (p.gym) s += lay(y(gymTop), y(hikeTop), GYM, !p.swim);
    if (p.swim) s += lay(y(tot), y(gymTop), SWIM, true);
    s += `</g>`;
